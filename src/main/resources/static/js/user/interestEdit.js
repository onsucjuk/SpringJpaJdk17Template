$(document).ready(function () {
    ...
    $("#btnInterestEdit").on("click", function () {
        ...
        $.ajax({
            url : "/user/udateInterest",
            type : "post",
            dataType: "JSON",
            data: { "interestSeq" : interestSeq,
                "seoulLocationCd" : guValue,
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let induSortList = document.getElementsByClassName("indutySort");

    for(let i =0; i < induSortList.length; i++) {

        let text = induSortList[i].innerText

        console.log("text : " + text)

        if(text===editSortNm){

            induSortList[i].id = "defaultIndutyClicked";
    ...
    let indutySelectSortList;

    if(editSortNm==="외식업") {
        let smallInduty = document.getElementById("foodInduty")
        indutySelectSortList = smallInduty.querySelectorAll(".indutySelectSort");
    }
    else if(editSortNm==="서비스업"){
        let smallInduty = document.getElementById("serviceInduty")
        indutySelectSortList = smallInduty.querySelectorAll(".indutySelectSort");
    }
    else {
        let smallInduty = document.getElementById("retailInduty")
        indutySelectSortList = smallInduty.querySelectorAll(".indutySelectSort");
    }


    for(let i =0; i < indutySelectSortList.length; i++) {

        let text = indutySelectSortList[i].innerText.trim()

        console.log("text : " + text)

        if(text===editInduNm){
            indutySelectSortList[i].classList.add("clicked");
        }
    }

    let guSelect = document.getElementById("guSelect");
    guSelect.value = editSeq;
